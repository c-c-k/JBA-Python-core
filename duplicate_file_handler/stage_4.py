...
"""DUPLICATE FILE HANDLER - stage 4: Delete them all

Finds and handles duplicate files in the tree of a given root dir.
Stage 4 assignment requirements:
    - Carry over stage 3 requirements:
        ...
        - Print each group of same sized files in the sort order that
            ...
            the subgroups doesn't matter), the format should be:
                ...
                {FILE ID}. {FILE PATH}
                ...
    - Ask the user if they want to delete some duplicate files.
        - Only accept the answers "yes" or "no", keep asking until
            a valid answer is given.
        - In case the user answers 'no' terminate the program.
    - Ask the user for the identifiers of the files they wish to delete.
        - Input should contain only existing file identifiers separated
            by spaces.
        - In case of invalid input, print an error message and ask again.
    - Delete the files that the user selected.
    - Report the total of all the files that were deleted.
"""
...
MenuIdsList = typing.List[FileMenuId]
...
MSG_ASK_DELETE_DUPLICATES = "Delete files?"
...
MSG_SELECT_FILE_IDS = "Enter file numbers to delete:"
...
MSG_SORT_MENU = (
    ...
    "1. Descending\n"
    "2. Ascending")
MSG_TOTAL_SIZE = "Total freed up space: {SIZE} bytes"
...
def query_delete_files() -> bool:
    while True:
        print(MSG_ASK_DELETE_DUPLICATES)
        choice = input()
        if choice == CHOICE_YES:
            return True
        elif choice == CHOICE_NO:
...
def select_delete_file_menu_ids(file_menu_ids: FileMenuIds):
    selected_file_menu_ids = {}
    while True:
        print(MSG_SELECT_FILE_IDS)
        try:
            selection = [int(menu_id) for menu_id in input().split()]
        except ValueError:
            selection = []
        for menu_id in selection:
            try:
                selected_file_menu_ids[menu_id] = file_menu_ids[menu_id]
            except KeyError:
                pass
        if len(selection) == 0:
            print(MSG_ERR_WRONG_FORMAT)
        else:
            break
    file_menu_ids.clear()
    file_menu_ids.update(selected_file_menu_ids)


def delete_selected_files(
        files_menu_ids: FileMenuIds,
        files_dataset: FilesDataset) -> FileSize:
    total_deleted_size = 0
    for dataset_index in files_menu_ids.values():
        os.remove(files_dataset[dataset_index].path)
        total_deleted_size += files_dataset[dataset_index].size
    return total_deleted_size


def print_total_deleted_size(total_deleted_size: FileSize):
    print(MSG_TOTAL_SIZE.format(SIZE=total_deleted_size))
...
def main():
    ...
    # Query the user if they wish to delete some files.
    delete_files = query_delete_files()

    # Terminate program if file deletion is not needed.
    if not delete_files:
        return

    # Query the user for the files to be deleted.
    select_delete_file_menu_ids(file_menu_ids)

    # Delete the selected files while recording their total size.
    total_deleted_size = delete_selected_files(file_menu_ids, files_dataset)

    # Print the total deleted size.
    print_total_deleted_size(total_deleted_size)
